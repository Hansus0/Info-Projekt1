...
def get_block(size):
    """
    Dungeon-Block: nimmt ein Tile aus dem Terrain-Sheet und tönt es kühl-grau,
    damit nichts nach Wiese/Grass aussieht.
    """
...
def update_ceiling(objects, block_size, offset_x, ceiling_y):
    """
    Hält eine durchgehende Decke in einem Band um die Kamera – wie update_ground, nur oben.
    """
    margin = WIDTH
    ...
    existing = set(obj.rect.x for obj in objects if isinstance(obj, Block) and obj.rect.y == ceiling_y)

    for obj in objects[:]:
        if isinstance(obj, Block) and obj.rect.y == ceiling_y and obj.rect.x not in needed:
            objects.remove(obj)

    for x in sorted(needed - existing):
        objects.append(Block(x, ceiling_y, block_size))


def generate_pillars(x_min, x_max, block_size, ground_y, ceiling_y, density=0.15):
    """
    Erzeugt vertikale Pfeiler-/Wandstücke zwischen Decke und Boden.
    density 0..1 steuert, wie oft ein Pfeiler im Chunk auftaucht.
    """
    placed = []
    step = block_size
    for x in range(x_min, x_max, step):
        if random.random() < density:
            gap_blocks = random.choice([1, 2])
            gap_start = random.randint(3, max(4, (ground_y - ceiling_y) // block_size - 4))
            y = ceiling_y
            while y < ground_y:
                on_gap = (y // block_size) in range(gap_start, gap_start + gap_blocks)
                if not on_gap:
                    placed.append(Block(x, y, block_size))
                y += block_size
    return placed
...
def update_ground(objects, block_size, offset_x, ground_y)
        update_ceiling(objects, block_size, offset_x, ceiling_y):
    """
...
def generate_cubes(x_min, x_max, num_cubes, block_size, occupied, ground_y, player_rect, max_vertical_gap=220, min_gap=None):
	...
	while len(placed) < num_cubes and attempts < max_attempts and xs:
		...
		length = random.choice([1,1,2,2,2,3])
...
def main():
    ...
    new_cubes = generate_cubes(start_gen, end_gen, initial_batch, block_size, occupied, ground_y, player.rect, max_vertical_gap=block_size * 3)
    if new_cubes:
        cubes.extend(new_cubes)
        objects.extend(new_cubes)
            pillars = generate_pillars(left_start, next_gen_left, block_size, ground_y, ceiling_y, density=0.12)
            if pillars:
                objects.extend(pillars)
            pillars = generate_pillars(next_gen_right, next_gen_right + GENERATE_CHUNK, block_size, ground_y, ceiling_y, density=0.12)
            if pillars:
                objects.extend(pillars)
    ...
    while run:
        ...
        while next_gen_right < offset_x + GENERATE_AHEAD:
            new_cubes = generate_cubes(next_gen_right, next_gen_right + GENERATE_CHUNK, CUBE_BATCH, block_size, occupied, ground_y, player.rect, max_vertical_gap=block_size * 3)
            if new_cubes:
                cubes.extend(new_cubes)
                objects.extend(new_cubes)
        ...
        while next_gen_left > offset_x - GENERATE_AHEAD:
            ...
            new_cubes = generate_cubes(left_start, next_gen_left, CUBE_BATCH, block_size, occupied, ground_y, player.rect, max_vertical_gap=block_size * 3)
            if new_cubes:
                cubes.extend(new_cubes)
                objects.extend(new_cubes)
