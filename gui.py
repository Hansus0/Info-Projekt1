...
class UI:
    def __init__(self, player, objects):
        ...
        self.abilities = [
            ...
            {"name": "Stomp", "key": "E", "cooldown": 0, "max_cd": self.player.STOMP_COOLDOWN},
        ]
    ...
    def draw_minimap(self, win, offset_x):
        ...
        # Draw points below the minimap
        font = pygame.font.SysFont("arial", 20)
        points_text = font.render(f"Points: {self.points}", True, (255, 255, 255))
        points_x = self.minimap_rect.x + (self.minimap_rect.width - points_text.get_width()) // 2
        points_y = self.minimap_rect.y + self.minimap_rect.height + 5
        win.blit(points_text, (points_x, points_y))
    ...
    def draw(self, win, offset_x):
        ...
        for i, ab in enumerate(self.abilities):
            ...
            if ab["name"] == "Dash":
                ...
                if self.player.stomping:
                    cd_txt = "Active"
                    color = (255, 255, 0)  # Yellow for active
                else:
                    current_cd = round(self.player.stomp_cooldown_timer, 1)
                    is_ready = current_cd >= ab['max_cd']
                    cd_txt = "Ready" if is_ready else f"{current_cd}/{ab['max_cd']}"
                    color = (0,255,0) if is_ready else (255,0,0)
            else:
                ...
                cd_txt = "Ready"
                color = (0,255,0)
            win.blit(font2.render(cd_txt, True, color),
