...
class Player(pygame.sprite.Sprite):
	...
	def loop(self, fps, dt):
		...
		# Default behavior when not holding or dashing (keeps original frame-based gravity)
...
def handle_ledge_grab(player, objects, dt, block_size):
	...
	if player.holding:
		...
		# Release if right button is released
	...
	# Not holding: require right mouse button held to start hold, and player must be airborne
	if not right_pressed:
		return

	# prevent immediate re-grab after releasing/jumping from a hold
